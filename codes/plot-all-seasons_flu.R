...
for (iyear in 1:nyears) {
	...
	epiList[[iyear]] = as.matrix(mydata$fit$raw) * rbind(as.matrix(mydata$fit$PerFlu)/100, matrix(0, nrow=mydata$nweeks - mydata$nweeksData, ncol=mydata$fit$nregions))
	epiNatl[[iyear]] = mydata$model$raw * c(mydata$model$PerFlu/100, rep(0,mydata$nweeks-mydata$nweeksData))
...
pdf(file=paste0("all-regions-EW",mydata$weeks[mydata$nweeksData],"-flu.pdf"), width=14, height=8)
...
for (iregion in 1:nregion) {
	...
	for (iyear in 1:nyears) {
		...
		if (iyear == 1) {
			...
			plot(1:nweeks,epi[1:nweeks, iregion], xaxt = "n", xlab = "", ylab = "% Flu", type = "l", col = col[iyear], lwd = 2,ylim=c(0,max(pk_valMAT[,iregion])),main=myName)
		} else {
...
dev.off()
#dev.new()
pdf(file=paste0("national-EW",mydata$weeks[mydata$nweeksData],"-flu.pdf"))
	...
	for (iyear in 1:nyears) {
		...
		if (iyear == 1) {
			...
			plot(1:nweeks, epi[1:nweeks], xaxt = "n", xlab = "", ylab = "% Flu", type = "l", col = col[iyear], lwd = 2, ylim = c(0, max(unlist(epiNatl))), main = myName)
		} else {
