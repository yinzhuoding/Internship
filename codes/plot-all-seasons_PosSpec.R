...
for (iyear in 1:nyears) {
	...
	epiList[[iyear]] = rbind(as.matrix(mydata$fit$PerFlu), matrix(0, nrow=mydata$nweeks - mydata$nweeksData, ncol=mydata$fit$nregions))
	epiNatl[[iyear]] = c(mydata$model$PerFlu, rep(0,mydata$nweeks-mydata$nweeksData))
...
pdf(file=paste0("all-regions-EW",mydata$weeks[mydata$nweeksData],"-PosSpec.pdf"), width=14, height=8)
...
for (iregion in 1:nregion) {
	...
	for (iyear in 1:nyears) {
		...
		nweeks = length(weeks)
		if (iyear == nyears) {
			epi[(mydata$nweeksData+1):mydata$nweeks,] = NA	
		}
		...
		if (iyear == 1) {
			...
			plot(1:nweeks,epi[1:nweeks, iregion], xaxt = "n", xlab = "", ylab = "% Specimens Positive", type = "l", col = col[iyear], lwd = 2,ylim=c(0,max(pk_valMAT[,iregion])),main=myName)
		} else {
			lines(1:nweeks,epi[1:nweeks, iregion], col = col[iyear], lwd = 2)
		}
...
dev.off()
#dev.new()
pdf(file=paste0("national-EW",mydata$weeks[mydata$nweeksData],"-PosSpec.pdf"))
	...
	for (iyear in 1:nyears) {
		...
		if (iyear == 1) {
			...
			plot(1:nweeks, epi[1:nweeks], xaxt = "n", xlab = "", ylab = "% Positive Specimens", type = "l", col = col[iyear], lwd = 2, ylim = c(0, max(unlist(epiNatl))), main = myName)
		} else {
			lines(1:nweeks, epi[1:nweeks], col = col[iyear], lwd = 2)
		}
