...
for (iyear in 1:nyears) {
	...
	epiList[[iyear]] = mydata$fit$sh
	epiNatl[[iyear]] = mydata$model$sh
	if (mydata$nweeksData < mydata$nweeks) {
	  epiList[[iyear]][(mydata$nweeksData+1):mydata$nweeks,] = 0
	  epiNatl[[iyear]][(mydata$nweeksData+1):mydata$nweeks] = 0
	}
...
pdf(file=paste0("all-regions_sh-EW",mydata$weeks[mydata$nweeksData],".pdf"), width=14, height=8)
...
for (iregion in 1:nregion) {
	...
	for (iyear in 1:nyears) {
		...
		pk_wk[iyear] = which.min(epi[, iregion])
		pk_wk[iyear] = weeks[pk_wk[iyear]]
		pk_val[iyear] = min(epi[, iregion],na.rm=TRUE)
		pk_val[iyear] = round(pk_val[iyear], digits = 5)
	...
	legend("bottomleft",legend=c(pk_wk,pk_wk_ave), bty = "n",text.col=c(col[1:nm1],'black'))
	legend("bottomright", legend=c(pk_val,pk_val_ave), bty = "n",text.col=c(col[1:nm1],'black'))
...
dev.off()
#dev.new()
pdf(file=paste0("national_sh-EW",mydata$weeks[mydata$nweeksData],".pdf"))
	...
	for (iyear in 1:nyears) {
		...
		if (iyear == 1) {
			...
			plot(1:nweeks, epi[1:nweeks], xaxt = "n", xlab = "", ylab = "SH (kg/kg)", type = "l", col = col[iyear], lwd = 2, ylim = c(0, max(unlist(epiNatl))), main = myName)
		} else {
			lines(1:nweeks, epi[1:nweeks], xaxt = "n", xlab = "", ylab = "SH (kg/kg)", type = "l", col = col[iyear], lwd = 2)
		}


		pk_wk[iyear] = which.min(epi)
		pk_wk[iyear] = weeks[pk_wk[iyear]]
		pk_val[iyear] = min(epi, na.rm = TRUE)
		pk_val[iyear] = round(pk_val[iyear], digits = 5)
	...
	legend("bottomleft", legend = pk_wk, bty = "n",text.col=col[1:nm1])
	legend("bottomright", legend = pk_val, bty = "n",text.col=col[1:nm1])
	axis(1, at = 1:nweeks, weeks)
