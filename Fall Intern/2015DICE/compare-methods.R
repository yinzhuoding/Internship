...
#csv_and_pdfDir = paste0("~/Documents/Share/",FY,"-season/csv_and_pdf/")
dataDir = paste0("~/Documents/Share/")
csv_and_pdfDir = paste0("~/Documents/Share/csv_and_pdf/")
...
for (imodel in 1:nmodels) {
	for (ip in 1:nprior) {
		if (Temp[ip] == 1) {
			subDir <- paste0(dataDir,"cdc-prior", prior[ip], "-nweeks-", nweeksFit)
		} else {
			subDir <- paste0(dataDir,"cdc-prior", prior[ip], "-sig", Temp[ip], "-nweeks-", nweeksFit)
		}
