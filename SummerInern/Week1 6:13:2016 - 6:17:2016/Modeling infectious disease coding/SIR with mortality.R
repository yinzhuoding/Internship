...
## This code assumes Density-Dependent Transmission
...
SIR.mort = function(time, state, pars) {
  with(as.list(c(state,pars)), {
    dX = nu - beta * X0 * Y0 - miu * X0
    dY = beta * X0 * Y0 - (gamma+miu)/(1-rho) * Y0
    dZ = gamma * Y0 - miu * Z0
...
out = ode(func = SIR.mort, y = yini, parms = pars, times = times)
